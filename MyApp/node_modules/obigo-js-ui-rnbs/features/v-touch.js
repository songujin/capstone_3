...
const gestures = ['doubletap', 'tap', 'pan', 'pinch', 'press', 'rotate', 'swipe']
const directions = ['up', 'down', 'left', 'right', 'horizontal', 'vertical', 'all']

export const VueTouch = {
  config: {},
  customEvents: {
    doubletap: {type: 'tap', taps: 2, event: 'doubletap', threshold: 200, interval: 400, posThreshold: 200},
    twofingertap: {type: 'tap', taps: 1, pointers: 2, event: 'twofingertap', threshold: 200, posThreshold: 200},
    twofingerdoubletap: {type: 'tap', taps: 2, pointers: 2, event: 'twofingerdoubletap', threshold: 200, interval: 400, posThreshold: 200},
    twofingerswipe: {type: 'swipe', pointers: 2, event: 'twofingerswipe'},
    twofingerpan: {type: 'pan', pointers: 2, event: 'twofingerpan'}
  },
  registerCustomEvent: function (event, options) {
    options.event = event
    this.customEvents[event] = options
  },
    ...
    Vue.directive('touch', {
      ...
      inserted (el, binding) {
        ...
        if (typeof binding.value !== 'function') {
          mc.handler = null
          console.warn(
            '[vue-touch] invalid handler function for v-hammer: ' +
            binding.arg
          )
        } else {
