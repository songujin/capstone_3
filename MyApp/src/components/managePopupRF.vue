    ...
    <div class='contents'>
        <div class='popup'>
            ...
            <div class='distance'>
                ...
                <div class='odometer'><vue-numeric-input v-model="userInputKM" separator="," :min="0" :controls="false"></vue-numeric-input><p>{{ distance }}km</p>
                </div>
            </div>
            <div class='btn'>
              <div class='btnBack'>
  ...
  methods: {
    ...
    go () {
      ...
      if (today.getTime() === setDate.getTime()) { // 오늘 부품을 교체할 시
        storage.saveRFTireKm(this.pastTotal)
        console.log(this.pastTotal)
      } else if (today.getTime() > setDate.getTime()) { // 과거에 부품을 교체했을 시
        storage.saveRFTireKm(this.userInputKM)
      }

      storage.saveRFTireUpdate(1) // update를 했다는 의미
      this.$router.push('/rightFrontTire')
    }
  }
}
