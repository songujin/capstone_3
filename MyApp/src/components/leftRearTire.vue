    ...
    <div class='contents'>
        ...
        <div class='manage'>
            ...
            <div class='detail'>
                ...
                <div class="status">
                  <p style="font-size: 22px;">Current status: {{ TireState }}</p>
                </div>
            </div>       
        </div>  
    </div> 
...
export default {
  ...
  created () {
    this.updateCnt = storage.loadLRTireUpdate()
  ...
  methods: {
    ...
    getOdometer (vo) {
      vo.get().then((odometer) => {
        ...
        this.pastTotal = storage.loadLRTireKm()
    ...
    getTire (vt) {
      vt.get().then((tire) => {
        console.log('get')
        this.TireState = tire[3].state
        this.pressure = tire[3].pressure
